...
func gen2() {
	...
	*/
	...
	imageFilename := fmt.Sprintf("images/test.png") // set the filename with the pid
	ibuf0.Png(imageFilename)                        // save the image as a PNG

	// create text file with the image filename
	textFilename := fmt.Sprintf("images/test.txt") // set the filename with the pid
	...
	infoString3 := fmt.Sprintf("min:%f max:%f huestart:%f huestop:%f", min, max, huestart, huestop)
